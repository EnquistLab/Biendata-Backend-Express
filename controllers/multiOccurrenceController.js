...
const getMultipleOccurrenceRecords = async (req, res) => {
    const { species } = req.query;

    if (!species) {
        return res.status(400).json({ error: 'At least one species name is required' });
    }

    // Split and trim species list
    let speciesList = species.split(',').map(s => s.trim()).filter(Boolean);

    // Limit to 100 species max
    if (speciesList.length > 100) {
        return res.status(400).json({ error: 'You can query up to 100 species at once.' });
    }
    ...
    try {
        ...
        const query = `
            ...
            WHERE scrubbed_species_binomial = ANY($1)
              AND is_geovalid = 1
        ...
        const result = await client.query(query, [speciesList]);

        await client.end();

        if (result.rows.length === 0) {
            return res.status(404).json({ message: `No data found for provided species.` });
        }

        res.status(200).json({ data: result.rows });
    } catch (error) {
        console.error('Error fetching multiple occurrence records:', error);
        res.status(500).json({ error: 'Internal Server Error', details: error.message });
    }
};

module.exports = { getMultipleOccurrenceRecords };
